...
bool PrimaryIndustry::CanOverbuild(Industry* other) const 
{
  ...
  if(other->IsBuilt() == false || static_cast<PrimaryIndustry*>(other)->resource_observer_->HasResource(other->resource_type())==false)//(temp->HasResource(resource_type()) == false))
  {
  ...
  {
    if(player() == other->player() && this->properties().tech_level < other->properties().tech_level)
      return true;
  }
  return false;
}
