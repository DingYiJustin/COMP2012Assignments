...
void Manager::printStatus()
{
    ...
    if (managerRegistry.size() != 0){
        ...
        for_each(managerRegistry.begin(), managerRegistry.end(), [](pair<string, BinarySearchTree*> i){cout<<" "<<i.first;});
    }   
}
...
Manager::~Manager()
{
    ...
    // Hint: It can be done using a single line of code
    // Hint2: The type of the elements in a map<A,B> is pair<A,B>
    for_each(managerRegistry.begin(), managerRegistry.end(), [](pair<string, BinarySearchTree*> i){delete i.second;});
}
